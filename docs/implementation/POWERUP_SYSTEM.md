...
| 등급 | 회복 주기 | 회복량 | ID |
|------|----------|--------|-----|
| 일반 | 8초 | 5 HP | `powerup_breathing_common` |
| 희귀 | 6초 | 8 HP | `powerup_breathing_rare` |
| 에픽 | 4초 | 12 HP | `powerup_breathing_epic` |

- **최대치**: 없음 (더 나은 등급으로만 교체 가능)
- **기본값**: 없음 (비활성)
- **특징**:
  - 이미 보유 중일 때 더 낮은 등급 획득 시 무시
  - 더 높은 등급 획득 시 자동 업그레이드
...
| 6 | 강체 | 방어 | -3% | -8% | -15% | 80% |
| 7 | 호흡 | 방어 | 8s/5 HP | 6s/8 HP | 4s/12 HP | 없음 |
| 8 | 경신 | 유틸 | +3% | +7% | +15% | 200% |
