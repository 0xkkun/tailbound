...
1. **구미호의 눈물** (Tier 2, Rare, Debuff) - 투사체 적중 시 10% 확률로 필드몹 매혹 (최대 10마리, 5초)
2. **처형인의 도끼** (Tier 3, Epic, Offensive) - 적 체력 20% 이하 시 즉시 처형
...
#### 🗡️ 처형인의 도끼 (Executioner's Axe) ✅ **구현 완료**

```typescript
효과:
- 일반 요괴(field) 체력 20% 이하 시 즉사 (100% 확률)
- named, boss 제외

시너지: 마무리 특화
색상: 다크 레드 (0x8b0000)
카테고리: offensive
구현: onHit 이벤트에서 체력 비율 체크 → executeDamage 적용

시각 효과:
  - 빨간 X 마크 (50px, 검은 외곽선 포함)
  - 1.5배 확대 + 45도 회전 애니메이션
  - 0.5초간 표시 후 페이드아웃
  - GSAP back.out(2) 이징

파일: src/game/artifacts/list/ExecutionerAxeArtifact.ts
```
...
export class EliteSpawnSystem {
  ...
  update(gameTime: number) {
    for (const spawnTime of this.spawnTimes) {
      ...
      // 스폰
      if (gameTime >= spawnTime && !this.spawnedElites.has(spawnTime)) {
